...
public class MatiereController{
	...
	@ResponseBody //Ne return pas une direction mais du contenu, ici un String
	public String filtrerMatiere(@RequestParam String recherche) 
	{
		String resultat="";
		List<Matiere> matieresRecherche = daoMatiere.findByLibelleContaining(recherche);
		if(matieresRecherche.isEmpty()) 
		{
			resultat+="<tr><td align='center' colspan='3'>AUCUNE MATIERE</td></tr>";
		}
		else 
		{
			for(Matiere m : matieresRecherche) 
			{
				resultat+="<tr><td>"+m.getId()+"</td><td>"+m.getLibelle()+"</td><td><a class='btn btn-warning' href='matiere/"+m.getId()+"'>Modifier</a><a class='btn btn-danger' href='matiere/delete/"+m.getId()+"'>Supprimer</a></td></tr>";
			}
		}
		return resultat;
	}
